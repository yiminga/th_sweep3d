...
# 预留评估数据
#取特征
X = data[:num,[0,1,2,3,4,5,16,17,18]]
X_scale=datalog(X)
# 标签
y=data[:num,19]
y=datalog(y)
...
def svrTest(c=13,epsilon=0.2,kernel='linear',gamma='auto' ):
    ...
    print (clf)
    joblib.dump(clf,'E:\\hpc\\th\\clf3.pkl')
# rf训练
...
def randomForestTest(n_estimators=10,max_features='auto',max_depth=None,min_samples_split=2,min_samples_leaf=1,oob_score=False,random_state=0):
    ...
    joblib.dump(regr, 'E:\\hpc\\th\\regr3.pkl')
    print(regr)
...
def ridegTest(alpha=0.5, copy_X=True, fit_intercept=True, max_iter=None,normalize=False, random_state=None, solver='auto', tol=0.001):
    ...
    for i in range(0, 10):
      ...
      normalize=False, random_state=None, solver='auto', tol=0.001)
        clf.fit(X_train, y_train)
    print(clf)
    joblib.dump(clf, 'E:\\hpc\\th\\ridge3.pkl')

svrTest(c=1,epsilon=0,kernel='linear')
#n_estimators=10：决策树的个数，越多越好，但是性能就会越差，
randomForestTest(n_estimators=4,max_features=8,min_samples_split=2,min_samples_leaf=2)
